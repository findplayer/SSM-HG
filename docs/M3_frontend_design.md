...
| R3 | 结构特征 dropout | **整通道（整列）置零、按图 Bernoulli(p)、p=丢弃率**（2026-09-16 统一语义，见手册 §8.6），非 elementwise | §3.2 |
...
| **先验 dropout**（大纲 4.1.4） | **正则**（随机） | `sv` 通道 `(N,1)` 乘图级 0/1 | `cat` 前 | 每图 Bernoulli(p)，p=丢弃率（默认 0.2） | 关闭（不置零） |
| **结构 dropout**（大纲 4.3.4） | **正则**（随机） | `struct` 通道 `(N,D)` 乘图级 0/1（**整通道**，R3） | `cat` 前 | 每图 Bernoulli(p)，p=丢弃率（默认 0.2） | 关闭（不置零） |
...
**Q2 — 结构特征 Dropout 形式？**
- 【选项】A elementwise `nn.Dropout`；B 整通道置零、按图 Bernoulli(p)（p=丢弃率）；
