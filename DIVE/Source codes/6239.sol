...
contract ScalpAI is ERC20, Ownable {
    ...
    function _setbuyfees(uint256 _marketing,uint256 _liquidity) external onlyOwner{
        require((_marketing+_liquidity) <= 10, "Must keep fees lower than 10%");
    ...
    function _setFees(uint256 _buy_marketing,uint256 _buy_liquidity,uint256 _sell_marketing,uint256 _sell_liquidity) external onlyOwner{
        require((_buy_marketing+_buy_liquidity) <= 10, "Must keep buy fees lower than 10%");
        require((_sell_marketing+_sell_liquidity) <= 10, "Must keep sell fees lower than 10%");
        buyMarketingFee = _buy_marketing;
        buyLiquidityFee = _buy_liquidity;
        TotalbuyFees = buyMarketingFee + buyLiquidityFee;
        sellMarketingFee = _sell_marketing;
        sellLiquidityFee = _sell_liquidity;
        TotalsellFees = sellMarketingFee + sellLiquidityFee;
    }
    ...
    function withdrawStuckETH() external onlyOwner {
        //withdraw ETH only if trading is disabled
        require(!tradingActive, "Trading is already active, cannot withdraw ETH.");
        bool success;
        (success,) = address(msg.sender).call{value: address(this).balance}("");
    }
    function updateDeadBlocks(uint256 _new) external onlyOwner{
        _blocks=_new;
    }
}