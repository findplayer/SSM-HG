...
contract CASniperRouter is Ownable2Step {
    using FixedPointMathLib for uint256;

    uint256 public sellFee = 1; // 2% in basis points (parts per 10,000)

    event FundsWithdrawn(uint256 amount);

    IUniswapV2Router02 public router;

    // router mainnet address = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    // router sepoila address = 0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008
    constructor() payable Ownable(0x215d20c4cFC58D2c40Aa4e0F43c47D53B6560E4a) {
        router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    }

    receive() external payable { }

    function swapExactETHForTokensSupportingFeeOnTransferTokens(address _tokenOut, uint256 amountOutMin, uint256 deadline) external payable {
        ...
        // perform the swap
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: msg.value }(amountOutMin, path, msg.sender, deadline);
    }

    function swapExactTokensForETHSupportingFeeOnTransferTokens(address _tokenIn, uint256 amountOutMin, uint256 amountInMax, uint256 deadline) external {
        // Define the path of the swap
        address[] memory path = new address[](2);
        path[0] = _tokenIn;
        path[1] = router.WETH();
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenToTransfer, amountOutMin, path, payable(address(this)), deadline);
