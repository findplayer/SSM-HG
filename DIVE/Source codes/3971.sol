...
contract XBXRouterV1 {
    ...
    constructor() public {
        admin = 0xD611CF2a27aA12174293d580afA18F00d28e38a8;
    }

    receive() external payable {}

    function xbx_admin(address _admin) external {
      require(msg.sender == admin, "Router: FORBIDDEN");
      admin = _admin;
    }

    function xbx_withdraw(uint amount) external {
      require(msg.sender == admin, "Router: FORBIDDEN");
      require(amount <= address(this).balance, "Router: INSUFFICIENT_BALANCE");
    ...
    function _swap(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? XBXLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            IUniswapV2Pair(XBXLibrary.pairFor(factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair pair = IUniswapV2Pair(
                XBXLibrary.pairFor(factory, input, output)
            );
            uint amountInput;
            uint amountOutput;
            {
    ...
    function xbx_swap(
        ...
        uint[] memory amounts;
        amounts = XBXLibrary.getAmountsOut(factory, ethAmount, path);
        if ((amountOut != 0) && (amounts[1] > amountOut)) {
            amounts = XBXLibrary.getAmountsIn(factory, amountOut, path);
        }
        require(amounts[1] > 0, "Insufficient token amount.");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(
            IWETH(WETH).transfer(
                XBXLibrary.pairFor(factory, path[0], path[1]),
                amounts[0]
            )
        );
    ...
    function xbx_sell(address tokenOut, uint256 amountIn) public {
        ...
        require(path[path.length - 1] == WETH, "XBXRouter: INVALID_PATH");
    ...
    function getAmountsOut(
        ...
        return XBXLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(
        ...
        return XBXLibrary.getAmountsIn(factory, amountOut, path);
    }
}
...
library XBXLibrary {
    ...
    function sortTokens(
        ...
        require(tokenA != tokenB, "XBXLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "XBXLibrary: ZERO_ADDRESS");
    }
    ...
    function quote(
        ...
        require(amountA > 0, "XBXLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "XBXLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "XBXLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "XBXLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "XBXLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "XBXLibrary: INVALID_PATH");
