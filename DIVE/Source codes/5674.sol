...
contract Strategy is BaseHealthCheck {
    ...
    function lenderKick(
        uint256 _index,
        uint256 _npLimitIndex
    ) external onlyVaultManagers {
        pool.lenderKick(_index, _npLimitIndex);
    }

    function withdrawBonds(uint256 _maxAmount) external onlyVaultManagers {
        pool.withdrawBonds(address(this), _maxAmount);
    }
    ...
    */
    ...
    function availableWithdrawLimit(
        ...
        return Math.min(TokenizedStrategy.totalIdle(), balanceOfWant());
    }
...
interface IStrategyInterface is IStrategy {
    ...
    function removeCollateral(uint256 _amount, uint256 _index) external;

    function lenderKick(uint256 _index, uint256 _npLimitIndex) external;

    function withdrawBonds(uint256 _maxAmount) external;
    ...
    function moveToLup(uint256 _amount) external;

    function fastExit(uint256 _amount) external;
}

contract AjnaStrategyFactory {
    address public management;
    address public sms;

    event AjnaStrategy(address indexed strategy, address indexed pool);
    ...
    function newAjnaStrategy(
        ...
        IStrategyInterface newStrategy = IStrategyInterface(
            address(new Strategy(address(_asset), _pool, _name))
        );

        newStrategy.setProfitMaxUnlockTime(0);
        newStrategy.setPerformanceFee(500);
        newStrategy.setDepositor(_vault);
        newStrategy.setPerformanceFeeRecipient(_vault);
        newStrategy.setPendingManagement(management);
        newStrategy.setEmergencyAdmin(sms);
        newStrategy.setKeeper(_keeper);

        emit AjnaStrategy(address(newStrategy), _pool);
