...
// File: degen/contracts/TestG.sol
...
error TestG__TransferFailed();
error TestG__NotEnoughBalance();
error TestG__Invalid_Address();
...
contract TestG is Ownable {
    ...
    function swapTokenToETH(address _tokenIn) external onlyValidator {
        ...
        if (amountIn == 0) revert TestG__NotEnoughBalance();
        if (
    ...
    function paymentDispersion() private {
        ...
        if (_amount > 0) {
            ...
            if (!success1) revert TestG__TransferFailed();
            (bool success2, ) = devWallet.call{value: _firstDivision}("");

            if (!success2) revert TestG__TransferFailed();
            (bool success3, ) = burn.call{value: _firstDivision}("");

            if (!success3) revert TestG__TransferFailed();
            (bool success4, ) = founder.call{value: _secondDivision}("");

            if (!success4) revert TestG__TransferFailed();
        }
    }
}