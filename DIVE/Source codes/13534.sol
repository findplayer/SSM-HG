...
contract USDCStakingContract54 {
...
if (lockupPeriod == 1) {
    ...
    require(amount >= 35 * 10**20 && amount <= 25**21, "Invalid deposit amount for 14-day lockup.");
    currentInterestRate = 180; // 1.8%
} else if (lockupPeriod == 2) {
    currentLockupPeriod = 30 * 1 days;
    require(amount >= 15**21 && amount <= 50 * 10**21, "Invalid deposit amount for 30-day lockup.");
    currentInterestRate = 380; // 3.8%
} else if (lockupPeriod == 3) {
    currentLockupPeriod = 60 * 1 days;
    require(amount >= 30 * 10**21 && amount <= 100 * 10**21, "Invalid deposit amount for 60-day lockup.");
    currentInterestRate = 800; // 8%
} else if (lockupPeriod == 4) {
    currentLockupPeriod = 90 * 1 days;
    require(amount >= 70 * 10**21 && amount <= 100 * 10**21, "Invalid deposit amount for 90-day lockup.");
