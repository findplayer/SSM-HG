...
// File contracts/KyzenGenesisItem.sol

pragma solidity ^0.8.0;

/**
 * @title Kyzen Genesis Item NFT
 * @author F.J.TAN, Vincent.Y.K.JIANG
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-Fungible Token Standard
 */
contract KyzenGenesisItem is ERC721, ERC721URIStorage, Mintable {
    ...
    constructor(
        ...
        ERC721("Kyzen Genesis Collection Items", "KYGC")
    ...
    /**
     * @param _tokenBaseURI New token base uri.
     * @dev Set new tokenBaseURI, This will change the metadata of already minted NFTs.
     * @notice This method can only be called by the owner of the contract.
     */
     ...
     * @dev NFTs often use a Uniform Resource Identifier (URI) for off-chain metadata,
     * which is a link to an external off-chain resource where the metadata for
     * that particular asset is stored, usually in a JSON format.
     ...
     * @param id The token id, must not exist.
     * @param blueprint Blueprint for each Token when minting on Immutable X,
     * this can be any value depending on how it is passed when Mint on L2
     * @dev This function will be called when the user initiates a withdrawal request on L2 Immutable X.
     */
    function _mintFor(
        address to,
        uint256 id,
        ...
        _safeMint(to, id);
        _setTokenURI(id, string(abi.encodePacked(tokenBaseURI, blueprint)));
    }
