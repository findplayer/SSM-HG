...
// File contracts/Staking/BatchDeposit.sol
...
contract BatchDeposit {
    ...
    address public constant SERVICE_CHARGE_ADDRESS = 0x23f74E45C709Fde36A08708B8f0DAA65Aa27FF2F;
    IDeposit private constant DEPOSIT_CONTRACT = IDeposit(DEPOSIT_CONTRACT_ADDRESS);

    event LogSendDepositLeftover(address indexed to , uint256 amount);
    event LogServiceCharge(address indexed from, address indexed to, uint256 amount);
    ...
    /// @param service_charge - Amount of service charge charged by the institution.
    function batchDeposit(
        ...
        require(
            msg.value >= DEPOSIT_AMOUNT.mul(pubkeys.length).add(service_charge),
            "#BatchDeposit batchDeposit(): Ether deposited needs to be at least: 32 * (parameter `pubkeys[]` length)."
        );
        // send service fee
        emit LogServiceCharge(msg.sender, SERVICE_CHARGE_ADDRESS, service_charge);
        payable(SERVICE_CHARGE_ADDRESS).sendValue(service_charge);
        uint256 deposited = service_charge;
        ...
        assert(deposited == DEPOSIT_AMOUNT.mul(pubkeys.length).add(service_charge));
