...
// File contracts/IFrogDescriptor.sol
...
pragma solidity ^0.8.13;
contract Frogs is ERC721Enumerable, Ownable, DefaultOperatorFilterer {
    ...
    constructor(IFrogDescriptor newDescriptor) ERC721("Block Hoppers", "FROG") {
...
function mint(uint32 count) external payable {
        ...
        require(count < 11, "Exceeds max per transaction (10).");
        uint256 nextTokenId = _owners.length;
        uint256 totalCost = count * 0.005 ether; // calculate the total cost based on count
        unchecked {
            require(nextTokenId + count < maxSupply, "Exceeds max supply.");
            require(msg.value == totalCost, "Please send exactly 0.005 ETH per NFT you wish to mint.");
        }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Block Hopper does not exist.");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 eyeSeed = 100 * (r % 18 + 10) + ((r >> 48) % 13 + 10);
        uint256 faceSeed = 100 * ((r >> 96) % 6 + 10) + ((r >> 144) % 8 + 10);
        uint256 bodySeed = 100 * ((r >> 144) % 17 + 10) + ((r >> 192) % 8 + 10);
        uint256 colSeed = 100 * ((r >> 192) % 20 + 10) + ((r >> 240) % 20 + 10);
        return 10000 * (10000 * (10000 * eyeSeed + faceSeed) + bodySeed) + colSeed;
    }
