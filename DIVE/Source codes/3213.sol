...
// File: contracts/WomenByUxio.sol
...
contract WomenByUxio is ERC721A, ERC2981, Ownable, ReentrancyGuard {

      uint public TOTAL_TOKENS = 52;
      uint public PRICE;
      uint public constant PRESALE_PRICE = 0.03 ether;
      uint public constant PUBLICSALE_PRICE = 0.08 ether;
      string public METADATA_PROVENANCE_HASH = "";

      string public baseURI = "https://www.antonuxio.com/collections/womenbyuxio/data/";
    ...
    constructor(address royaltyRecipient, uint96 royaltyPoints) ERC721A("Women by Uxio","WAU")  {
    ...
    function newTotalTokens(uint256 newTotal)  external payable onlyOwner{
    ...
    function mint(uint256 numTokens) external payable nonReentrant() {
        ...
        require(balanceOf(msg.sender) + numTokens <= 5, "Max 5 pieces per wallet.");
    ...
    function setDiamondList(address[] calldata addresses) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) {
            diamondList[addresses[i]] = 5; // Max 5 mints per wallet 
        }
    }
