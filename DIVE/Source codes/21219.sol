...
contract G3NES1SLABS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("G3NES1S LABS", "G3NES1S") {
        ...
        uint256 totalSupply = 1 * 1e10 * 1e10;
        supply += totalSupply;
    ...
    function enableDumpTax() external onlyOwner {
        buyBurnFee = 0;
        buyDevFee = 12;
        buyTotalFees = buyBurnFee + buyDevFee;
    ...
    function updateTransferMulti(uint256 newNum) external onlyOwner {
        require(newNum >= 1);
        transDigit = newNum;
        updateLimits();
    }

    function updateWalletMulti(uint256 newNum) external onlyOwner {
        require(newNum >= 1);
        walletDigit = newNum;
        updateLimits();
    }

    function updateDelayMulti(uint256 newNum) external onlyOwner{
        delayDigit = newNum;
    }
