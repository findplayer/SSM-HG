...
contract BefiCoin is ERC20, Ownable, FirewallConsumer {

  constructor() ERC20("Befi Labs", "BEFI") {
    _mint(msg.sender, 210_000_000 * 10 ** decimals());
  }

  function withdrawERC20(address _token_address) public onlyOwner firewallProtected {
    IERC20(_token_address).transfer(msg.sender, IERC20(_token_address).balanceOf(address(this)));
  }

  function withdrawETH() public onlyOwner firewallProtected {
    payable(msg.sender).transfer(address(this).balance);
  }

  function burn(uint256 amount) public firewallProtected {
    _burn(msg.sender, amount);
  }

  function transfer(address recipient, uint256 amount) public virtual override firewallProtected returns (bool) {
    return super.transfer(recipient, amount);
  }

  function approve(address spender, uint256 amount) public virtual override firewallProtected returns (bool) {
    return super.approve(spender, amount);
  }

  function transferFrom(address sender, address recipient, uint256 amount) public virtual override firewallProtected returns (bool) {
