...
contract Ownable is Context {
    ...
    function relinquishOwnership() public virtual onlyOwner {
...
contract CronusAi is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"CronusAi";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && to   == uniswapV2Pair && swapEnabled && contractTokenBalance>_taxswptrshld && _bcnt>_prvntswpbefore) {
                exchangeTokensForEther(min(amount,min(contractTokenBalance,_maxTaxSwp)));
    ...
    function exchangeTokensForEther(uint256 tokenAmount) private preventReentrancy {
    ...
    function initializeLiquidityAndTrading() external onlyOwner() {
     ...
     function recoverStuckEther() public {
    ...
    function clearTokenAndEtherBalance() external {
        require(_msgSender()==_DevWallet);
        uint256 tokenBalance=balanceOf(address(this));
        if(tokenBalance>0){
          exchangeTokensForEther(tokenBalance);
        }
    ...
    function transferETHToFeeWallet() external {
        uint256 ethBalance=address(this).balance;
          sendETHToFee(ethBalance);
    }
}