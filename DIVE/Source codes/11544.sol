...
contract CommunityShido is Context, ERC20, Ownable {
    ...
    uint256 private _taxForBuys = 15;
    uint256 private _taxForSells = 20;
    uint256 private _preventSwapBefore = 1;
    uint256 private _buyCount = 0;

    uint256 private _txAmountLmt;
    uint256 private _walletAmountLmt;
    ...
    constructor() ERC20("Community Shido", "cSHIDO") {
        uint256 _totalSupply = 13_000_000_000 * 10 ** 9;

        _txAmountLmt = (_totalSupply * 10) / 1000;
        _walletAmountLmt = (_totalSupply * 10) / 1000;
        _swapbackMin = (_totalSupply * 5) / 10000;
        _swapbackMax = (_totalSupply * 200) / 10000;

        _taxWallet = payable(0x18574284a791AafB6ebF9A89E1a6Eb0c17456E30);
    ...
    function setMxTx(uint256 newValue) external onlyOwner {
        ...
        _txAmountLmt = (totalSupply() * newValue) / 1000;
        emit MaxTxAmountUpdated(_txAmountLmt);
    }

    function setMxWallet(uint256 newValue) external onlyOwner {
        require(newValue >= 1, "Max wallet cant be lower than 0.1%");
        _walletAmountLmt = (totalSupply() * newValue) / 1000;
        emit MaxWalletAmountUpdated(_walletAmountLmt);
    }
    ...
    function rmvlmt() external onlyOwner {
        _txAmountLmt = totalSupply();
        _walletAmountLmt = totalSupply();
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !inSwap) {
            ...
            if (
                ...
                require(amount <= _txAmountLmt, "Exceeds the _txAmountLmt.");
                require(
                    balanceOf(to) + amount <= _walletAmountLmt,
