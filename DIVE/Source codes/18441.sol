/**
    ...
    https://t.me/foodogeth

    https://www.foodogeth.com

    https://twitter.com/FooDogETH

    

    Mint your Foo Dog NFT here:
    https://mint.foodogeth.com

    Stake your NFT here:
    https://stake.foodogeth.com
...
contract FooDog is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x0b9863A80D9ba177d65172aD4d647cE578b9f778;
    address public buybackWallet = 0x31508A248Af9a9c106FfAD7A80A142628e120457;
    ...
    uint256 public minimumTokensBeforeSwap = 5_000 * 10**_decimals;
    ...
    function setBuyFee(uint _newLP, uint _newMarket, uint _newBuyBack) public onlyOwner {
        require(_newLP.add(_newMarket).add(_newBuyBack) <= 25, "Total taxes cannot exceed 25%.");
        _buyLiquidityFee = _newLP;
        _buyMarketingFee = _newMarket;
        _buyBuyBackFee = _newBuyBack;
        totalBuy = _buyLiquidityFee.add(_buyMarketingFee).add(_buyBuyBackFee);
    }

    function setSellFee(uint _newLP, uint _newMarket, uint _newBuyBack) public onlyOwner {
        require(_newLP.add(_newMarket).add(_newBuyBack) <= 25, "Total taxes cannot exceed 25%.");
        _sellLiquidityFee = _newLP;
        _sellMarketingFee = _newMarket;
        _sellBuyBackFee = _newBuyBack;
        totalSell = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBuyBackFee);
    }

    function setWallets(address _market,address _BuyBack,address _liquidityRec) public onlyOwner {
