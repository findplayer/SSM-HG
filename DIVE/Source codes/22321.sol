...
contract CAWELON is Context, ERC20, Ownable {
    ...
    event CAWBurned(address _deadWallet, uint256 CAWtokenamount);
    ...
    function updateAirDrop (address[] calldata recipients, uint256 _unlockTime) public onlyOwner {
      for (uint256 i = 0; i < recipients.length; i++){
        airDropWalletsData[recipients[i]].unlockTime = block.timestamp.add(_unlockTime * 86400);
      }
    }
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount * (10**18) >= totalSupply() / 2000, "SwapTokensAtAmount must be greater than or equal to 0.05% of total supply"); 
        swapTokensAtAmount = amount * (10**18);
    }

    function setDevWallet(address payable wallet) external onlyOwner{
      require(wallet != devWallet, " The already has that address");
      require(wallet != address(0), "Cannot set zero address");
       
        emit DevWalletUpdated(wallet, devWallet);
    ...
    function setCAWCollectWallet(address payable wallet) external onlyOwner{
      require(wallet != cawCollectWallet, " The already has that address");
      require(wallet != address(0), "Cannot set zero address");
       
        emit CAWCollectWalletUpdated(wallet, cawCollectWallet);

         cawCollectWallet = wallet;
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newAmount) external onlyOwner {
        dividendTracker.updateMinimumTokenBalanceForDividends(newAmount);
    }
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 24 hours && newClaimWait <= 7 days, "Dividend_Tracker: claimWait must be updated to between 24 hours and 7 days");
