...
contract AZUKI404 is ERC404, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        uint8 _decimal
    ) ERC404("Azuki 404", "AZUKI", _decimal, _initialSupply, _owner) {
    ...
    function _transfer(
        ...
        if(!whitelist[from]){
            ...
            require(userSelllimit[from] <= sellLimit, "Tx limit reached, try a lower amount");
        }
        if(!whitelist[to]){
            userBuylimit[to] += amount;
            require(userBuylimit[to] <= buyLimit, "Tx limit reached, try a lower amount");
        }
