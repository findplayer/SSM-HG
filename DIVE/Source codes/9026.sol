...
contract TokenSwap is ERC721Holder, Ownable {
    ...
    function pullInvader() internal {
        // Get the next available Invaders NFT to be swapped
        uint256 nextTokenId = invaders.tokenOfOwnerByIndex(address(this), 0);

        // Transfer the selected Invaders NFT to the caller
        invaders.safeTransferFrom(address(this), msg.sender, nextTokenId);
    }

    function sendNftToDead(
