...
// Creating a new contract that inherits from ERC721 and Ownable contracts
contract Everyone_Is_art is ERC721, Ownable {
  using Strings for uint256;

  // Setting up variables for metadata, supply, and minting
  string private uriPrefix = "ipfs://everyone_is_art/";
  ...
  // Constructor sets up the initial MetadataUri and token metadata
  constructor() ERC721("EveryoneIsArt", "AIE") {
    setMetadataUri("ipfs://art_is_everyone");
  }

  // Function to mint a new token
function mint() public {
    require(!paused, "The contract is paused!");
    require(!addressMinted[msg.sender], "Address has already minted!");

    addressMinted[msg.sender] = true;

    uint256 tokensToMint = 2;

    // Check if there is enough supply for 2 tokens, otherwise mint only 1
    if (currentIndex + tokensToMint > maxSupply) {
        tokensToMint = 1;
    }

    _mintLoop(msg.sender, tokensToMint);
}
    ...
    returns (string memory)
  {
    ...
    // Functions to set various contract variables (only callable by the contract owner)
 
  // Function to set whether the token metadata has been revealed or not
  ...
  // Function to set the URI prefix
  function setUriPrefix(string memory _uriPrefix) public onlyOwner {
    uriPrefix = _uriPrefix;
  }

  // Function to set the URI suffix
  ...
  // Internal function to mint tokens in a loop
  ...
  // Function to set the maximum supply of tokens
  ...
  // Function to withdraw all the contract's balance to a specific address
