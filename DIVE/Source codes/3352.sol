...
library MakerDaiDelegateLib {
    ...
    enum Action {WIND, UNWIND, PAYDOWN}

    //Curve:
    ICurveFi internal constant curve = ICurveFi(0xbEbc44782C7dB0a1A60Cb6fe97d0b483032FF1C7);
    ...
    function emergencyPullPosition(uint256 _collateralToPull, uint256 cdpId) external {
        wipeAndFreeGem(gemJoinAdapter, cdpId, _collateralToPull, 0);
    }
    ...
    function paydown(
        uint256 _collateralAmountToSwap,
        uint256 _flashloanAmount,
        uint256 _acceptedSlippageBPS,
        uint256 cdpId,
        address flashmint
    ) public {
        if (balanceOfCdp(cdpId, ilk_yieldBearing) == 0){
        ...
        //flashloan only up to maximum allowed:
        flashloanAmount = Math.min(flashloanAmount, flashloanMaximum);
        //flashloan only up to _flashloanAmount input
        flashloanAmount = Math.min(flashloanAmount, _flashloanAmount);
        bytes memory data = abi.encode(Action.PAYDOWN, cdpId, _collateralAmountToSwap, _flashloanAmount, _acceptedSlippageBPS);
        //Always flashloan entire debt to pay off entire debt:
        _initFlashLoan(data, flashloanAmount, flashmint);
    }

    function _paydown(uint256 cdpId, uint256 flashloanAmount, uint256 flashloanRepayAmount, uint256 collateralAmountToSwap, uint256 acceptedSlippageBPS) public {
        //Make sure that chosen collateral amount is not bigger than ALL collateral:
        collateralAmountToSwap = Math.min(collateralAmountToSwap, balanceOfCdp(cdpId, ilk_yieldBearing));
        //Make sure that chosen debt repayment with flashloan amount is not bigger than entire debt:
        uint256 debtRepayment = Math.min(flashloanAmount, debtForCdp(cdpId, ilk_yieldBearing).add(1));
        //Check allowance for repaying borrowToken Debt
        _checkAllowance(daiJoinAddress(), address(borrowToken), debtRepayment);
        //repay some or all debt with flashloan & take out chosen amount of collateral:
        wipeAndFreeGem(gemJoinAdapter, cdpId, collateralAmountToSwap, debtRepayment);
        //GUNI Collateral is now in the strategy:
        //Burn the yieldBearing token to unlock DAI and USDC:
        yieldBearing.burn(Math.min(collateralAmountToSwap, balanceOfYieldBearing()), address(this));
        //Amount of otherToken (USDC) after burning:
        uint256 otherTokenBalance = balanceOfOtherToken();
        //approve Curve DAI/USDC
        _checkAllowance(address(curve), address(otherToken), otherTokenBalance);
        uint256 minAmountOut = otherTokenBalance.mul(1e12).mul(MAX_BPS.sub(acceptedSlippageBPS)).div(MAX_BPS); //calculate amount out in DAI decimals from USDC amountIn, accounting for set slippage
        curve.exchange(1, 0, otherTokenBalance, minAmountOut); //accept only swaps according to set slippage
        require(balanceOfWant() >= flashloanRepayAmount, "not enough DAI to repay flashloan");

        //in case of DAI surplus above flashloan repayment, we pay down more debt
        uint256 currentMakerDebt = debtForCdp(cdpId, ilk_yieldBearing);
        if(balanceOfWant() > flashloanRepayAmount && currentMakerDebt != 0){
            wipeAndFreeGem(gemJoinAdapter, cdpId, 0, Math.min(currentMakerDebt.add(1), balanceOfWant().sub(flashloanRepayAmount))); //Paying off the full debt it's common to experience Vat/dust reverts: we circumvent this with add 1 Wei to the amount to be paid
        }
    }
...
contract Strategy is BaseStrategy {
    ...
    function emergencyPaydown(uint256 _collateralAmountToSwap, uint256 _flashloanAmount, uint256 _acceptedSlippageBPS, uint256 _lowestAcceptedFinalWantBalance, uint256 _lowestAcceptedFinalEstimatedTotalAssets) external onlyVaultManagers {
        require(_acceptedSlippageBPS <= 100_00, "acceptedSlippageBPS above 100_00");
        require(_lowestAcceptedFinalEstimatedTotalAssets > 0, "accepted finalEstimatedTotalAssets should be > 0");
        MakerDaiDelegateLib.paydown(_collateralAmountToSwap, _flashloanAmount, _acceptedSlippageBPS, cdpId, flashmint);
        require(want.balanceOf(address(this)) >= _lowestAcceptedFinalWantBalance, "final want balance too low!");
        require(estimatedTotalAssets() >= _lowestAcceptedFinalEstimatedTotalAssets, "final estimatedTotalAssets too low!");
    }

    function emergencyPaydownALL(uint256 _acceptedSlippageBPS, uint256 _lowestAcceptedFinalWantBalance) external onlyVaultManagers {
        require(_acceptedSlippageBPS <= 100_00, "acceptedSlippageBPS above 100_00");
        uint beforeBalance = want.balanceOf(address(this));
        require(_lowestAcceptedFinalWantBalance > beforeBalance, "accepted final want balance <= beforeBalance!");
        require(_lowestAcceptedFinalWantBalance > 0, "accepted final want balance should be > 0");
        MakerDaiDelegateLib.paydown(balanceOfMakerVault(), balanceOfDebt().add(1), _acceptedSlippageBPS, cdpId, flashmint); //Paying off the full debt it's common to experience Vat/dust reverts: we circumvent this with add 1 Wei to the amount to be paid
        uint256 finalBalance = want.balanceOf(address(this));
        require(finalBalance > beforeBalance, "final want balance <= beforeBalance!");
        require(finalBalance >= _lowestAcceptedFinalWantBalance, "final want balance too low!");
    }

    function emergencyPullPosition(uint256 _collateralToPull) external onlyGovernance {
        MakerDaiDelegateLib.emergencyPullPosition(_collateralToPull, cdpId);
    }
    ...
    function onFlashLoan(
        ...
        } else if (action == Action.PAYDOWN) {
            MakerDaiDelegateLib._paydown(_cdpId, amount, amount.add(fee), _wantAmountInitialOrRequested, _collateralizationRatio);
        }
