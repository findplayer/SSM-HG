...
modifier nonEmpty(uint256[] memory tokenIds) {
    require(tokenIds.length > 0, "Array of tokenIds cannot be empty");
    _;
}
...
uint256 vaultId = vaultInfo.length;
    vaultInfo.push(
    VaultInfo({nft: _nft, token: _token, name: _name})
);

    // Emit the event with the relevant information
    emit VaultAdded(vaultId, address(_nft), address(_token), _name);
}

function removeVault(uint256 vaultId) external onlyOwner {
    require(vaultId < vaultInfo.length, "Invalid vaultId");

    // Before removing a vault, ensure all staked NFTs are unstaked and rewards are claimed
    for (uint256 i = 0; i < vaultInfo.length; i++) {
        if (i != vaultId) {
            vaultInfo[i - (i > vaultId ? 1 : 0)] = vaultInfo[i];
        }
    }

    // Reduce the vaultInfo array length by 1
    vaultInfo.pop();

    // Emit an event to inform that the vault has been removed
...
for (uint i = 0; i < tokenIds.length; i++) {
        ...
        // Update balance and remove token ID from owner's token array
        ownerBalances[_pid][account] -= 1;
       _removeTokenId(ownerTokens[_pid][account], tokenId);

        emit NFTUnstaked(account, tokenId, block.timestamp);

        // Unstake NFT and delete the stake
        delete vault[_pid][tokenId];
        vaultInfo[_pid].nft.transferFrom(address(this), account, tokenId);
    }
}
...
function _removeTokenId(uint256[] storage tokenArray, uint256 tokenId) internal {
    ...
    // Replace the token to remove with the last token in the array
    tokenArray[index] = tokenArray[tokenArray.length - 1];
...
function _claim(address account, uint256[] calldata tokenIds, uint256 _pid, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
        ...
        earned += (rewardRate * (block.timestamp - staked.timestamp));
        vault[_pid][tokenId] = Stake({
            owner: account,
            tokenId: uint24(tokenId),
            timestamp: uint48(block.timestamp)
...
function updateVault(
    ...
    VaultInfo storage vaultToUpdate = vaultInfo[vaultId];
    vaultToUpdate.nft = _nft;
    vaultToUpdate.token = _token;
    vaultToUpdate.name = _name;
}

function setStakableNFT(uint256 vaultId, IERC721 _nft) external onlyOwner {
    require(vaultId < vaultInfo.length, "Invalid vaultId");
    vaultInfo[vaultId].nft = _nft;
}

function rescueTokens(IERC20 token, address recipient, uint256 amount) external onlyOwner {
    require(recipient != address(0), "Cannot send tokens to the zero address");
...
function onERC721Received(
    ...
    require(operator == address(this), "Cannot send NFTs to Vault directly");
