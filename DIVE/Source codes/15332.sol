...
contract LiquidityLock is Owned {
  ...
  constructor(address _deployer) Owned(msg.sender) {
    ...
    require(positionManager != address(0), "zero");
...
//   INonfungiblePositionManager public immutable uniswapV3PositionManager;
  ...
  uint256 public tokenSupply = 1_000_000e18;
  uint256 public benchmarkSupply = tokenSupply / 100;
  ...
  constructor(address delete_) Owned(msg.sender) {
    if (block.chainid == 1) {
        positionManager = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
    ...
    tokenSupply = ERC20(_delete).totalSupply();
    benchmarkSupply = tokenSupply / 100;
  ...
  function causeChaos() external {
    ...
    uint256 _deleteBalance = ERC20(_delete).balanceOf(address(this));

    if (_deleteBalance > 0) {
      ERC20(_delete).transfer(address(0xdead), _deleteBalance);
    }
    ...
    for (uint256 i = 0; i < validityChecks; i++) {
      ...
      if (i % 2 == 0) {
        ...
        _deleteBalance = ERC20(_delete).balanceOf(address(this));
      }
    }
    _deleteBalance = ERC20(_delete).balanceOf(address(this));
    swap(address(_delete), weth, _deleteBalance);

    uint256 burnedSupply = ERC20(_delete).balanceOf(address(0));
