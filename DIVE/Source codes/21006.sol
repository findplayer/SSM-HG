...
contract PENNYstaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when PENNYstaking reward ends.
    uint256 public bonusEndBlock;

    // The block number when PENNYstaking reward starts.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && tokenStaked != 0) {
            ...
            uint256 PENNYstakingReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(PENNYstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 PENNYstakingReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(PENNYstakingReward.mul(PRECISION_FACTOR).div(tokenStaked));
        lastRewardBlock = block.number;
    }
