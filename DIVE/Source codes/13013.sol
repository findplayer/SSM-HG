/**
https://t.me/EZFreelanceErc
https://www.EZFreelance.org
*/
...
contract EZFREELANCE is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"EZ Freelance";
    string private constant _symbol = unicode"EZF";
    ...
    constructor () payable {
        _marketingWallet = payable(0xA03f8DDFb6Ab5B0ABd135168D326EafA73545d5D);
        _developmentWallet = payable(0xA03f8DDFb6Ab5B0ABd135168D326EafA73545d5D);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "EZF: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function updateBuyTax(uint256 _marketing,uint256 _development,uint256 _lp) external onlyOwner {
        ...
        require(buyTax.totalPc < 400,"EZ: Buy tax can not greater than 40%");
    }
    function updateSellTax(uint256 _marketing,uint256 _development,uint256 _lp) external onlyOwner {
        ...
        require(sellTax.totalPc < 550,"EZ: Sell tax can not greater than 55%");
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "EZF: Approving from the zero address");
        require(spender != address(0), "EZF: Approving to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "EZF: Transfer from the zero address");
        require(to != address(0), "EZF: Transfer to the zero address");

        if (from != owner() && to != owner()) {
            require(tradingOpen,"EZF: trading != true");
            require(!isBlacklisted[from] && !isBlacklisted[to]);

            TaxStructure storage _tax = ZERO;
            if(!_isOutFromFee[to]){
                require((_balOwned[to] + amount) <= _maxWalletAmount,"EZF: Max Wallet Limit");
                require(amount <= _maxTxAmount,"EZF: Max TxAmount Limit");
