...
/*
    ...
    • All $FRENZ Uniswap trading tax is distributed to rooms
    • Global chat
    ...
    Web: https://frenzroom.world
    X: https://x.com/FrenzRoom_ERC
    Tg: https://t.me/frenzroom_official
...
contract FRENZ is Context, IERC20, Ownable {
    using SafeMath for uint256;

    string name_ = unicode"FrenzRoom";
    string symbol_ = unicode"FRENZ";
    ...
    uint256 sellFrenzLiquidityFee_ = 0;
    uint256 sellFrenzMarketingFee_ = 25;
    uint256 sellFrenzDevFee_ = 0;
    uint256 sellFrenzFee_ = 25;

    uint256 _maxTx = 15 * 10**6 * 10**9;
    uint256 _maxWallet = 15 * 10**6 * 10**9;
    uint256 _swapThreshold = 10**4 * 10**9;

    uint256 currentLiquidityFee_ = 0;
    uint256 currentMarketingFee_ = 25;
    uint256 currentDevelopmentFee_ = 0;
    uint256 currentTotalFee_ = 25;

    bool swapping_;
    bool swapEnabled_ = true;
    bool _maxTxLifted = false;
    bool _isMaxWalletLifted = true;

    uint256 buyFrenzLiquidityFee_ = 0;
    uint256 buyFrenzMarketingFee_ = 25;
    uint256 buyFrenzDevFee_ = 0;
    uint256 buyFrenzFee_ = 25;
    ...
    constructor(address _taxAddress) {
        ...
        buyFrenzFee_ = buyFrenzLiquidityFee_.add(buyFrenzMarketingFee_).add(buyFrenzDevFee_);
        sellFrenzFee_ = sellFrenzLiquidityFee_.add(sellFrenzMarketingFee_).add(sellFrenzDevFee_);
    ...
    function removeLimits() external onlyOwner {
        _maxTx = totalTokenSupply_;
        _isMaxWalletLifted = false;
        buyFrenzMarketingFee_ = 2;
        sellFrenzMarketingFee_ = 2;
        buyFrenzFee_ = 2;
        sellFrenzFee_ = 2;
    }
    ...
    function _checkSwaps(address from, address to, uint256 amount) internal {
        ...
        bool isNotReentrance = !swapping_ && isLiquidityAddr_[to] && swapEnabled_;
    ...
    function _transferInternal(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (swapping_) {
            ...
            if (!excludedMaxTx_[sender] && !excludedMaxTx_[recipient]) {
                require(amount <= _maxTx, "Transfer amount exceeds the maxTx.");
            }
            ...
            if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
                ...
                addAmount = getFrenzAmount_(sender, recipient, amount);
            }
    ...
    function transferFrenzETH_(address payable recipient, uint256 amount) private {
    ...
    function getFeeAmount(address from, address to, uint256 amount) internal view returns (uint256) {
        if (isLiquidityAddr_[from]) {
            return amount.mul(buyFrenzFee_).div(100);
        } else if (isLiquidityAddr_[to]) {
            return amount.mul(sellFrenzFee_).div(100);
        }
    }

    function getFrenzAmount_(address sender, address receipient, uint256 amount) internal returns (uint256) {
    ...
    function swapBackFrenz_(uint256 tokenAmount) private lockSwap {
        ...
        if (amountETHMarketing_ > 0) {
            transferFrenzETH_(marketingWallet_, amountETHMarketing_);
        }

        if (amountETHDevelopment_ > 0) {
            transferFrenzETH_(teamAddress_, amountETHDevelopment_);
        }
    }
