...
contract FACEX is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public TOTAL_MAX_SUPPLY = 3000;
  ...
  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'FACEX: The caller is another contract');
    _;
  }
  ...
  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "FACEX: Mint limit exceeded");
    require(saleStarted, "FACEX: Sale has not started");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "FACEX: More ETH required");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "FACEX: Not enough ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "FACEX: Not enough ETH");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "FACEX: Transfer failed");
  }
