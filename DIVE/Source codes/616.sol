 ...
 */
contract RVFMigration is Ownable, ReentrancyGuard {
    address public rvfToken;
    address public rvfV2Token;
     ...
     * @param _rvfV2Token Address of the RVFv2 token contract.
    ...
    function swap(uint256 amount) external nonReentrant {
        ...
        require(
            IERC20(rvfToken).balanceOf(msg.sender) >= amount,
            "Insufficient RVF balance"
        );
        require(
            IERC20(rvfToken).allowance(msg.sender, address(this)) >= amount,
            "Allowance not enough"
        );

        // Check contract liquidity
        require(
            IERC20(rvfV2Token).balanceOf(address(this)) >= amount,
            "Contract out of liquidity"
        );

        // Transfer RVF tokens to token collector
        IERC20(rvfToken).transferFrom(msg.sender, tokenCollector, amount);

        // Transfer swap fee to fee collector
        IERC20(rvfV2Token).transfer(feeCollector, feeAmount);

        // Transfer RVFv2 tokens to user
        IERC20(rvfV2Token).transfer(msg.sender, amount - feeAmount);
     ...
     */
    function emergencyWithdrawal(uint256 amount)
        external
        onlyOwner
        nonReentrant
    {
        ...
        IERC20(rvfV2Token).transfer(owner(), amount);
