...
contract CNFT is AccessControl, ERC721, ICNFT {
    ...
    function isMintingEligible(bytes memory encodedVm) public view override returns (address to) {
        ...
        require(_ownerToTokenId[to] == 0 && to != address(0), "invalid recipient");
    }

    function mintInternal(address recipient) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 currentReserveID = _currentReserve + 1;
        require(currentReserveID <= _resMax, "internal supply exceeds max");
        _ownerToTokenId[recipient] = currentReserveID;
        _currentReserve = currentReserveID;
        _safeMint(recipient, _currentReserve);
    }

    function mint(bytes memory encodedVm) external override {
        uint256 currentID = _currentId + 1;
        address to = isMintingEligible(encodedVm);
        uint256 current = _resMax + currentID;
        _ownerToTokenId[to] = current;
