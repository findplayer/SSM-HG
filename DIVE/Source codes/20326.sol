...
//   INonfungiblePositionManager public immutable uniswapV3PositionManager;
  ...
  constructor() Owned(msg.sender) {
    ...
    deployer = msg.sender;
    flood = new Flood(msg.sender, tokenSupply);
    lock = new LiquidityLock(msg.sender);

    ERC20(flood).approve(address(positionManager), type(uint256).max);
    WETH(payable(weth)).approve(address(positionManager), type(uint256).max);

    flood.renounceOwnership();
  ...
  function causeChaos() external {
    if (lock.nftId() == 0) {
      ...
      pool = IUniswapV3Pool(factory.getPool(weth, address(flood), poolFee));
      ERC20(flood).approve(address(pool), type(uint256).max);
      WETH(payable(weth)).approve(address(pool), type(uint256).max);
      require(address(pool) != address(0));
      token0 = pool.token0();
    }

    uint256 floodBalance = flood.balanceOf(address(this));

    if (floodBalance > 0) {
      flood.transfer(address(0), floodBalance);
    }
    ...
    for (uint256 i = 0; i < validityChecks; i++) {
      tokenIn = i % 2 == 0 ? weth : address(flood);
      tokenOut = i % 2 == 0 ? address(flood) : weth;
      swapBalance = i % 2 == 0 ? wethBalance : floodBalance;
      if (swapBalance > 0) {
      ...
      if (i % 2 == 0) {
        ...
        floodBalance = flood.balanceOf(address(this));
      }
    }
    floodBalance = flood.balanceOf(address(this));
    swap(address(flood), weth, floodBalance);

    uint256 burnedSupply = flood.balanceOf(address(0));
