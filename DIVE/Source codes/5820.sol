...
contract MultiRewardsVaultStrategy is BaseHealthCheck, TradeFactorySwapper {
    using SafeERC20 for ERC20;
    ...
    constructor(
        ...
        vault = IStrategy(_vaultAddress);

        require(multiRewards.stakingToken() == _vaultAddress);
        require(vault.asset() == _asset);

        ERC20(_asset).safeApprove(_vaultAddress, type(uint256).max);
        ERC20(_vaultAddress).safeApprove(
    ...
    function _deployFunds(uint256 _amount) internal override {
        ...
        multiRewards.stake(balanceOfVault());
    }
    ...
    function _freeFunds(uint256 _amount) internal override {
        ...
        if (shares > vaultBalance) {
            unchecked {
                multiRewards.withdraw(
                    Math.min(shares - vaultBalance, balanceOfStake())
                );
            }
        ...
        returns (uint256 _totalAssets)
    {
        ...
        _totalAssets =
            balanceOfWant() +
            vault.convertToAssets(balanceOfStake());
    }

    function availableWithdrawLimit(
        address
    ) public view virtual override returns (uint256) {
        return vault.convertToAssets(vault.maxRedeem(address(multiRewards)));
    }
...
contract MultiRewardsVaultStrategyFactory {
