...
contract RollGame is Ownable, ERC20 {
...
contract RollBot is Ownable {

    address public rewardWallet;
    address public admin;

    RollGame public immutable bettingToken;
    ...
    constructor(address payable _bettingToken, uint256 _rewardRate, uint256 _burnRate, address _rewardWallet) {
        ...
        burnRate = _burnRate;
        bettingToken = RollGame(_bettingToken);
    }
    ...
    function setMinimalBet(uint256 _minimalBet) public onlyOwner {
    ...
    function startGame(int64 _tgChatId, address[] memory _players, uint256[] memory _bets) public returns (uint256[] memory) {
        ...
        for (uint16 i = 0; i < _bets.length; i++) {
            require(_bets[i] >= minimalBet, "Bet is smaller than the minimum");
            betTotal += _bets[i];
        }
        for (uint16 i = 0; i < _bets.length; i++) {
            betTotal -= _bets[i];
    ...
    function finishGame(int64 _tgChatId, uint16[] memory _losers) public {
        require(msg.sender == admin);
