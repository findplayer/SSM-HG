...
contract presale {
    ...
    uint256 public minmumPurchaseInEth = 0.01 ether;
    ...
    function buyTokenBNB(address _ref, uint256 amount) public {
        require(presaleStatus == true, "Presale : Presale is finished");
        require(
            amount >= minmumPurchaseInBNB,
            "Presale : amount must be greater than minimum purchase"
        );
        require(amount > 0, "Presale : Unsuitable Amount");
        require(soldToken <= SupplyPerPhase, "All Sold");

        BNB.transferFrom(msg.sender, fundReceiver, amount);
        ...
        soldToken = soldToken + (numberOfTokens);
        amountRaisedBNB = amountRaisedBNB + (amount);

        users[msg.sender].Bnb_balance += amount;
    ...
    function bnbToUsdt(
        ...
        uint256 price = getLatestPriceBnb();

        // Compute the value of bnbAmount in USDT
        bnbValueInUsd = (bnbAmount * price) / (priceFeedBNB.decimals());
    }
    ...
    function minPurchase(
        uint256 _minmumPurchaseInEth,
        uint256 _minmumPurchaseInUSDT,
        uint256 _minmumPurchaseInBNB
    ) public onlyOwner {
        uint256 minmumPurchaseInEth = _minmumPurchaseInEth;
        uint256 minmumPurchaseInUSDT = _minmumPurchaseInUSDT;
        uint256 minmumPurchaseInBNB = _minmumPurchaseInBNB;
    }
