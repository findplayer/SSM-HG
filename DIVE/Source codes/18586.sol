...
abstract contract SemiFungibleERC741 is Ownable {
    event Approval(
    ...
    /// @dev Addresses ignoreNFT from minting / burning for gas savings (pairs, routers, etc)
    mapping(address => bool) public excludeNFT;
    ...
    function setExcludeNFT(address target, bool state) public onlyOwner {
    ...
    /// @notice Function for ERC20 transfers, only callable by the ERC20Relayer
    function transferERC20(
        address sender,
        address to,
        uint256 amount
    ...
    /// @notice Function for ERC20 balance, only callable by the ERC20Relayer
    function balanceOfERC20(address account) public view returns (uint256) {
        return _balance[account];
    }

    /// @notice Function for ERC20 allowance, only callable by the ERC20Relayer
    function transferFromERC20(
        address sender,
        address from,
        address to,
    ...
    /// @notice Function for ERC20 approval, only callable by the ERC20Relayer
    function approveERC20(
        address sender,
        address spender,
        uint256 amount
...
contract ERC741 is SemiFungibleERC741 {


    constructor() SemiFungibleERC741("ERC741", "PUPPIES") {

        baseTokenURI = '';

        _erc20Supply = 1000 * 10 ** 18;
        erc721totalSupply = 1000;
        address tokenReceiver = 0xC80594DEc342a7a75D1Bdb3A729f007510d1641A;
        _balance[tokenReceiver] = _erc20Supply;
        ...
        ).createPool(WETH, address(this), 10000);

        excludeNFT[tokenReceiver] = true;
        excludeNFT[v3Pool] = true;
        excludeNFT[0x0CF98f5C524e461319adF92A5f93EBa2abDd0B4e] = true;
        excludeNFT[0x75A089547c714049887BFbcc183fFE034B046307] = true;
        excludeNFT[0x05855823F1Efc3ACb419EEA1951a56A0A9990Ae5] = true;

        erc20Relayer.emitTransfer(address(0), tokenReceiver, _erc20Supply);
    }

    function setExcludeNFTPublic(bool status) external {
        if (_balance[_msgSender()] > 0) {
            revert InvalidSetWhitelistCondition();
        }
        excludeNFT[_msgSender()] = status;
    }

    bool public isSame = true;
    function setIsSame(bool s) public onlyOwner{
        isSame = s;
    }
    function tokenURI(uint256 id) public override virtual returns (string memory) {
        if (isSame){
            return baseTokenURI;
        }else{
