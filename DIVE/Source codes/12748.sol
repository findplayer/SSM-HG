...
contract AFFE is ERC20, Ownable {
    ...
    function setFees(uint256 _liqFee, uint256 _markFee, uint256 _devFee) public onlyOwner() {
        require(_liqFee.add(_markFee).add(_devFee) <= 8, "tax too high");
    ...
    function setMaxTransactionAmount(uint256 _maxTxAmount) public onlyOwner {
        // Ensure the new value does not exceed 1.5% of the total supply.
        require(_maxTxAmount >= totalSupply().mul(15).div(1000), "value too high");
    ...
    function setMaxWalletToken(uint256 _newValue) external onlyOwner {
        ...
        require(maxWalletToken >= totalSupply().mul(15).div(1000), "value too low");
