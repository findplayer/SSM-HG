...
contract StakedSecurix is IERC20, Domain {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    using BoringERC20 for IERC20;

    string public constant symbol = "stakedSecurix";
    string public constant name = "Staked Securix Tokens";
    uint8 public constant decimals = 18;
    uint256 public override totalSupply;
    uint256 private constant LOCK_TIME = 24 hours;

    IERC20 public immutable Securix;
    ...
    function stake(uint256 amount) public returns (bool) {
        ...
        uint256 totalTokens = Securix.balanceOf(address(this));
        ...
        Securix.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function _burn(
        ...
        uint256 amount = (shares * Securix.balanceOf(address(this))) / totalSupply;
        users[from].balance = user.balance.sub(shares.to128()); // Must check underflow
        totalSupply -= shares;

        Securix.safeTransfer(to, amount);

        emit Transfer(from, address(0), shares);
    }

    function unstake(address to, uint256 shares) public returns (bool) {
