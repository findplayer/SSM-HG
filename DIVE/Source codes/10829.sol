...
contract Pomadoge is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1777777777 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Pomadoge";
    string private constant _symbol = "POMD";
    ...
    uint256 public numTokensSellToAddToLiquidity = 1777777 * 10**18;
    ...
    function setBuyTaxFee(uint256 taxFee,uint256 liquidityFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyLiquidityFee = liquidityFee;
        require(_buyTaxFee + _buyLiquidityFee <= 15, "Total fees exceed 15%");
    }

    function setSellTFee(uint256 taxFee, uint256 liquidityFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellLiquidityFee = liquidityFee;
        require(_sellTaxFee + _sellLiquidityFee <= 15, "Total fees exceed 15%");
    }
