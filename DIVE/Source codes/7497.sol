...
/// |__   _| |_| |__   _|   
///    |_|  \___/   |_|    
/// Hybrid token/nft on the DN404 standard with some improvements               
...
contract COOLESTCATS is DN404, Ownable {
    ...
    function setSkipNFTByOwner(address a, bool state) public onlyOwner {
        ...
        uint256 launchLimit = 500 *  10 ** 18;
        uint256 buyLimit = 500 *  10 ** 18;

    mapping(address => uint256) internal userLaunchLimit;
    mapping(address => uint256) internal userBuyLimit;

    function _transfer(address from, address to, uint256 amount) internal override virtual{

        if (from != owner()) {
            userLaunchLimit[from] += amount;
            require(userLaunchLimit[from] <= launchLimit, "Trying to sell too many during launch phase");
        }

        if (to != owner()) {
            userBuyLimit[to] += amount;
            require(userBuyLimit[to] <= buyLimit, "Transaction limit reached, try a lower buying amount");
        }

        super._transfer(from, to, amount);
    }

    function setLimit(uint256 _buylimit, uint256 _launchLimit) public onlyOwner{
        buyLimit = _buylimit;
