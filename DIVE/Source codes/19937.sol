...
/**
 * @title Raries NFT
 * @notice Sample DN404 contract that demonstrates selling NFTs to the general public.
 * The underlying call still mints ERC20 tokens, but to the end user it'll appear as a standard NFT mint.
 */
contract Raries is DN404, Ownable {
    ...
    uint256 public constant MAX_SUPPLY = 22156;
    ...
    function mint(uint256 amount) public payable isValidMint(publicPrice, amount) {
        if((balanceOf(msg.sender)/1e18) + amount > maxMint) revert InvalidMint();
