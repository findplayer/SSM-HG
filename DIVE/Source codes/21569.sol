...
contract ERC1155Mintable is Ownable, ERC1155Supply{
    string public name;
    string public symbol;
    address public signer;
    mapping(uint256 => string) _uris;

    constructor(address _signer, string memory _name, string memory _symbol, string memory _uri0) ERC1155(""){
        signer = _signer;
        name = _name;
        symbol = _symbol;
        _uris[0] = _uri0;
    ...
    function setURI(uint256 id, string calldata _uri) external onlyOwner{
        _uris[id] = _uri;
        emit URI(_uri, id);
    }
    ...
    function mintByOwner(uint256 id, uint256 amount) external onlyOwner{
        _mint(msg.sender, id, amount, "");
    }

    function mint(uint256 id, uint256 amount, uint256 deadline, bytes calldata signature) external{
        require(block.timestamp <= deadline, "timeout");
        bytes32 hash = keccak256(abi.encodePacked(block.chainid, address(this), msg.sender, id, amount, deadline));
        require(ECDSA.recover(hash, signature) == signer, "sign error");
        _mint(msg.sender, id, amount, "");
    }
}