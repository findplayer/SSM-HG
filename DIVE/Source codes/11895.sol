...
contract TokenExchange {
    ...
    uint256 public maxTxBuyAmount = 10000 * 10 ** 6;
    uint256 public maxTxSellAmount = 750000 * 10 ** 18;
    uint256 public minTxBuyAmount = 10 * 10 ** 6;
    ...
    function depositTokenA(uint256 amount) public {
        ...
        IERC20_USDT(tokenA).transferFrom(msg.sender, address(this), amount);

        uint256 feeAmount = amount * feeRate / 1000; // calculate fee amount
        uint256 inputAmount = amount - feeAmount; // calculate input amount
        uint256 outputAmount = (inputAmount * exchangeBuyRate / buyDecimals) * 10 ** 12; // calculate output amount
    ...
    function depositTokenB(uint256 amount) public {
        ...
        uint256 feeAmount = amount * feeRate / 1000; // calculate fee amount
        uint256 inputAmount = amount - feeAmount; // calculate input amount
        uint256 outputAmount = (inputAmount / exchangeSellRate * sellDecimals) / 10 ** 12; // calculate output amount

        IERC20_USDT(tokenA).transfer(msg.sender, outputAmount);
    ...
    // Widthdraw shit tokens from contract
    function withdrawShitTokens(address _token, uint256 _amount) public {
        require(msg.sender == owner, 'Only owner can call this function');

        IERC20_USDT(_token).transfer(owner, _amount);
    }
    ...
    function withdrawTokenATaxFee() public {
        ...
        IERC20_USDT(tokenA).transfer(owner, amount);
