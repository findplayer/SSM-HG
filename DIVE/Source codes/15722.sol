...
interface IRewarder {
    function onVVSReward(uint256 _pid, address _user, uint256 _amount) external;

    function pendingToken(uint256 pid, address user) external view returns (address rewardToken, uint256 amount);
}

contract CraftsmanV2 is OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    event AddPid(uint256 indexed pid, address lpToken, IRewarder[] rewarders);
    ...
    function deposit(uint256 _pid, uint256 _amount) external whenNotPaused {
        ...
        require(address(pool.lpToken) != address(0), "the _pid has not been created yet");
    ...
    function add(IERC20Upgradeable _lpToken, IRewarder[] memory _rewarders) external onlyOwner {
        require(address(_lpToken) != address(0), "_lpToken cannot be zero address");
        require(lpTokenIndex[address(_lpToken)] == 0, "_lpToken already exists in a pool");
        uint256 _pid = poolInfo.length;
        poolInfo.push(PoolInfo({lpToken: _lpToken, rewarders: _rewarders}));
        lpTokenIndex[address(_lpToken)] = _pid;
        emit AddPid(_pid, address(_lpToken), _rewarders);
    }
    ...
    function pendingTokens(uint256 _pid, address _user) external view returns (address[] memory, uint256[] memory) {
        ...
        uint256[] memory pendingAmounts = new uint256[](rewardLength);
