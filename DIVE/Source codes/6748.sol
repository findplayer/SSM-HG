...
ParadigmTV is a decentralized Text-to-Video platform, powered by Bittensor ($TAO) technology.
...
/**
 * @title Paradigm TV Token Contract
 *
 * @notice Paradigm TV is an extended version of ERC-20 standard token that
 ...
 */
contract Paradigm is Ownable, IERC20Metadata, IERC20Errors, ICommonError {
    ...
    string private constant NAME = "Paradigm TV";
    string private constant SYMBOL = "PTV";
    ...
    uint256 public minSwap = 50 ether;
    uint256 public walletLimit = 1_000;
    uint256 public txnLimit = 2_000;

    address public projectOwner = 0xd299f1F380B8Dd3aaE2325621A8d946797911F6E;
    address public marketingReceiver = 0xf06d783caB8513f66C67766c1C7eceE229B26F28;
    ...
    /**
     * @notice Error indicating that balance exceed max transaction limit.
     */
    error MaxTxnLimitExceed(uint256 value, uint256 limit);
    ...
    /**
     * @notice Constructs the Paradigm TV contract and initializes both owner
    ...
    constructor() Ownable (msg.sender) {
        ...
        uint256 supply = 100_000 * 10**DECIMALS;
        uint256 preloadSupply = supply * 30_000 / FEEDENOMINATOR;
        uint256 mintingSupply = supply - preloadSupply;
        _mint(address(this), preloadSupply);
        _mint(msg.sender, mintingSupply);

        collectedFee.marketing += preloadSupply;
        totalFeeCollected += preloadSupply;
     ...
     * @param oldBuyFee The previous fee value before the update.
     * @param newBuyFee The new fee value after the update.
     * @param oldSellFee The previous fee value before the update.
     * @param newSellFee The new fee value after the update.
     * @param caller The address of the caller who updated the fee.
     * @param timestamp The timestamp when the fee update occurred.
     */
    event UpdateFee(string feeType, uint256 oldBuyFee, uint256 newBuyFee, uint256 oldSellFee, uint256 newSellFee, address caller, uint256 timestamp);
    ...
    /**
     * @notice Checks the max transaction limit for an address.
     *
     * @return The limit based on circulating supply of tokens.
     */
    function checkTxnLimit() internal view returns (uint256) {
        return circulatingSupply() * txnLimit / FEEDENOMINATOR;
    }
     ...
     * 
     * @param newMarketingFee The new marketing fee percentage for buy & sell transactions.
     * 
     ...
     */
    function updateFee(uint256 newMarketingFee) external onlyOwner {
        if (isFeeLocked) {
            revert FeeLocked();
        }
        ...
        if (newMarketingFee == buyFee.marketing && newMarketingFee == sellFee.marketing) {
            revert CannotUseAllCurrentValue();
        }
        uint256 oldBuyMarketingFee = buyFee.marketing;
        uint256 oldSellMarketingFee = sellFee.marketing;
        buyFee.marketing = newMarketingFee;
        sellFee.marketing = newMarketingFee;
        emit UpdateFee("buyfee & sellfee - Marketing", oldBuyMarketingFee, newMarketingFee, oldSellMarketingFee, newMarketingFee, msg.sender, block.timestamp);
    }
    ...
    /**
     * @notice Updates the exemption status for max transaction limit on a specific account.
     * 
     * @param user The address of the account.
     * @param newStatus The new exemption status.
     * 
     ...
     * for the `user` address.
     */
    function updateExemptTxnLimit(address user, bool newStatus) external onlyOwner {
        if (isExemptTxnLimit[user] == newStatus) { revert CannotUseCurrentState(newStatus); }
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if (isTxnLimitActive && !isExemptTxnLimit[from] && newValue > checkTxnLimit()) {
            revert MaxTxnLimitExceed(newValue, checkTxnLimit());
        }
