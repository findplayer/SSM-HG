...
interface IGatewayContract {
    function openGateway(address tokenOwner, uint256 tokenId) external;
}

contract NewBurnTwo is Ownable {
    IGatewayContract mythicArtifacts;
    ...
    constructor() {
        mythicArtifacts = IGatewayContract(Artifacts);
        tokenContract = IERC1155(Artifacts);
    }


    function BurnMyArtifact(uint256 artifactId) public  {
        // You can only burn an artifact 1-18 or 42
        // This should ensure the world matches the artifact id
        //
        require(
            (artifactId > 0 && artifactId <= 18) || artifactId == 42,
            "Not a valid Artifact"
        );

        // Burn the necessary artifact
        require(
            tokenContract.balanceOf(msg.sender, artifactId) >= 1,
            "TokenOwner does not own enough tokens!"
        );

        IGatewayContract gatewayContract = IGatewayContract(Artifacts);
        gatewayContract.openGateway(msg.sender, artifactId);
    }
    ...
    function SendNFTToDead(address nftContractAddress, uint256 tokenId) public {
        ...
        // Make sure we are the sender and own the nft
        // Check if the caller is the owner of the NFT
        address tokenOwner = nftContract.ownerOf(tokenId);
        require(msg.sender == tokenOwner, "Sender is not the owner of the NFT");


        // Approve the BurnTheNFTs contract to manage the NFT on behalf of the owner
        nftContract.approve(address(this), tokenId);

        // Call the safeTransferFrom function in the target NFT contract to transfer the NFT to the dead address
    ...
    function mintTest(uint256 legend, uint256 invader, uint256 artifact) public {
        SendNFTToDead(Legends, legend);
        SendNFTToDead(Invaders, invader);
        BurnMyArtifact(artifact);
    }
