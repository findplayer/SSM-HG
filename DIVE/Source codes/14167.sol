...
contract shiAIStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 stakedAmount;     // How many shiAI tokens the user has provided.
  ...
  constructor(address _token) {
    ...
    shiAIToken = _token;
    totalStakedAmount = 0;
    rewardRate = 9; // User will get 12% reward after lockup time.
  }
  ...
  function stakeToken(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(shiAIToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  ...
  function claim() external nonReentrant {
    ...
    require(IERC20(shiAIToken).balanceOf(address(this)) >= receiveAmount, "staking contract has not enough shiAI token");

    IERC20(shiAIToken).safeTransfer(msg.sender, receiveAmount);
    totalStakedAmount = IERC20(shiAIToken).balanceOf(address(this)).sub(receiveAmount);
