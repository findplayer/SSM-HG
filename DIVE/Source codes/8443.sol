...
contract DinoToken is ERC721, Mintable {
    ...
    constructor(
        ...
        uint256 _maxSupply,
        address _imxAddress
    ) ERC721(name, symbol) Mintable(msg.sender, _imxAddress) {
        maxSupply = _maxSupply;

        // Defaults
        isPaused = true; //Start Paused
        doValidateSender = true; //Validate message senders
        ethCost = 100000000000000000; //0.1eth default
        maxMintPerWallet = 20; //20 mint per wallet default
    }
    
    function mintNFTs(uint256 numberOfTokens) public payable {
        require(!doValidateSender || msg.sender == tx.origin, "Only real users can call this function.");
        require(numberOfTokens > 0, "Invalid number of tokens to mint.");
        require(msg.value >= ethCost * numberOfTokens, "Insufficient ETH to mint tokens.");
        require(maxSupply >= _tokenIdCounter.current() + numberOfTokens, "Max Supply reached.");
        require(!isPaused, "Minting Paused.");
        require(
            _mintedTokensPerWallet[msg.sender] + numberOfTokens <= maxMintPerWallet,
            "Maximum mint quantity per wallet exceeded."
        );
    ...
    function _mintToken(address recipient) private {
        ...
        uint256 newItemId = _tokenIdCounter.current(); //will start at 1
        _safeMint(recipient, newItemId);
        _mintedTokensPerWallet[recipient]++;
    }
    ...
    function giveaway(address[] memory recipients, uint256[] memory amounts) public onlyOwner {
    ...
    /// IMX L2 Minting
    event ImxMintFor(address user, uint256 imxid, uint256 tokenid);
    ...
    function _mintFor(
       ...
       _mintToken(user);
       emit ImxMintFor(user, id, _tokenIdCounter.current());
    }
    ///
    ...
    function setEthCost(uint256 _ethCost) public onlyOwner {
        ethCost = _ethCost;
    }
    ...
    function setValidateSender(bool _validateSender) public onlyOwner {
        doValidateSender = _validateSender;
    }
