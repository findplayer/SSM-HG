...
contract Cryptovreneli is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  event BuyNFT(address indexed user, uint256 tokenId);
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    function mint(uint256 numberOfNFT) public  onlyOwner{
        ...
        require(supply + numberOfNFT <= maxSupply, "Exceeds maximum Originators supply");
        uint256 _mintedSupply = totalMinted;
        if(!isApprovedForAll(msg.sender, address(this))){
            setApprovalForAll(address(this), true);
        }
        for (uint256 i = 1; i <= numberOfNFT; i++) {
    ...
    function buyNFT(uint256 tokenId) external payable{
        require(this.ownerOf(tokenId) == owner(), "NFT not available in seller wallet");
        require(msg.value >= cost, "insufficient fund for purchase");
