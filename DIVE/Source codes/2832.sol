/**
    Twitter:    https://twitter.com/Furnace404
 */
...
library LibString {
    ...
    function contains(string memory subject, string memory search)
        internal
        pure
        returns (bool)
    {
...
abstract contract DN404 {
    ...
    uint256 internal constant _WAD = 1;
    ...
    ///
    /// Initializes account `a` AddressData if it is not currently initialized.
    function _addressData(address a)
        internal
        virtual
...
contract Furnace404 is DN404, Ownable {
    ...
    constructor() {
        ...
        _name = "Furnace 404";
        _symbol = "Furnace";
        maxWallet = 1024 * 10**18; // 0.2 per wallet

        address mirror = address(new DN404Mirror(msg.sender));
        _initializeDN404(1024 * 10**18, msg.sender, mirror); // 1 token max supply
    }
        ...
        returns (bool)
    {
        if (!getSkipNFT(to)) {
            require(
                ...
                "This transfer will exceed receiver max holding amount of tokens"
            );
        }
        ...
        returns (string memory result)
    {
        if (bytes(_baseURI).length != 0) {
            ...
            result = "https://i.ibb.co/c1gCWQh/Furnace404.png";
        }
    }

    function sendMultiple(address[] memory to, uint256[] memory amount)
        public
        onlyOwner
    {
