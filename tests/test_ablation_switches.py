...
| 关闭先验 Dropout | **★ 发现实现与大纲相反，见 `test_prior_dropout_*`** | ❌ 待裁定 |
...
# ============================================================ 先验 dropout：★ 已知缺口
def test_prior_dropout_mask_is_keep_probability_not_drop_probability():
    """⚠ **当前实现**：掩码为 1 的概率 = `prior_p`，即 `prior_p` 是**保留率**而非丢弃率。

    与大纲 4.1.4 / 手册 §8.6 的散文「以概率 0.2 把 s_v **置 0**」**相反**：
    默认 `--prior-dropout 0.2` 实际置零 **80%** 的图。本测试钉住现状，防止无声漂移；
    处置见 `experiments/decisions.md` §25（**待用户裁定**）。
    """
    for p in (0.0, 0.2, 0.5, 1.0):
        m, _ = sample_dropout_masks(4000, prior_p=p, struct_p=p,
                                    generator=torch.Generator().manual_seed(0))
        assert m.mean().item() == pytest.approx(p, abs=0.03), \
            f"prior_p={p} 时掩码为 1 的比例应为 {p}（即实现视其为保留率）"


def test_prior_dropout_zero_currently_zeroes_everything():
    """⚠ 后果：`--prior-dropout 0` 会把 **每张图** 的 s_v 置零 ——

    而 5.4.1 的消融项正是「**关闭**先验 Dropout」。也就是说该消融目前无法按意图表达，
    跑出来等价于 `--ablate-sv`（另一项消融），会导致两个"不同"消融其实测的是同一件事。
    """
    m, _ = sample_dropout_masks(64, prior_p=0.0, struct_p=0.0,
                                generator=torch.Generator().manual_seed(0))
    assert torch.all(m == 0), "当前实现下 prior_p=0 → 全置零"


@pytest.mark.xfail(strict=True,
                   reason="实现与大纲 4.1.4 相反：`--prior-dropout 0` 应为「关闭」(全保留)，"
                          "实测全置零。修复会作废全部既有结果，待 decisions §25 裁定。")
def test_prior_dropout_zero_should_disable_dropout__documented_behaviour():
    """**按大纲应有的行为**（当前 xfail）：丢弃率为 0 ⇒ 掩码恒为 1。"""
    m, _ = sample_dropout_masks(64, prior_p=0.0, struct_p=0.0,
                                generator=torch.Generator().manual_seed(0))
    assert torch.all(m == 1), "按大纲语义，dropout 概率 0 应表示「不丢弃」"
