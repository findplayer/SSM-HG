...
契约（与 train/evaluate/ablation 三处共用；**不得 import dataset/model**，可独立单测）：
  - 主指标 **micro-F1（标签对级）**：对 `(样本, 类)` 展平后计算，即 sklearn `f1_score(average="micro")`；
    不逐类平均、不额外做标签组合。macro-F1 为参考指标，报告时须注明支撑构成。
...
def micro_f1(y, p) -> float:
    ...
    return float(f1_score(_as_numpy(y).ravel(), _as_numpy(p).ravel(),
...
def search_global_threshold(probs_val, y_val,
    ...
    """
    metric_fn = micro_f1 if metric == "micro_f1" else macro_f1
    y_np, p_np = _as_numpy(y_val), _as_numpy(probs_val)
    best_threshold = None
    ...
    for thr in candidates:                      # 升序遍历 → 并列自然保留较小阈值
        ...
        mif = float(f1_score(y_np.ravel(), preds.ravel(), average="micro", zero_division=0))
